        ...
        <div className="flex h-screen bg-slate-950 text-slate-50 font-sans overflow-hidden">
            ...
            <aside className="w-80 bg-slate-900/50 border-r border-slate-800 flex flex-col p-8 backdrop-blur-xl">
                ...
                <div className="mt-auto pt-8 border-t border-slate-800/50 space-y-4">
                    ...
                    <div
                        className={`p-6 rounded-3xl border relative overflow-hidden group transition-all duration-300 cursor-pointer ${updateStatus.available ? "bg-emerald-500/10 border-emerald-500/30 hover:bg-emerald-500/20" : "bg-gradient-to-br from-slate-900 to-slate-950 border-slate-800 hover:border-slate-700"}`}
                        onClick={() => updateStatus.available ? window.open(REPO_RELEASES_URL, "_blank") : setShowAboutModal(true)}
                        title="Ver información del proyecto y creador"
                    >
                        {updateStatus.available && <div className="absolute inset-0 bg-emerald-500/5 animate-pulse" />}
                        <div className="relative z-10">
                            <div className="flex items-center justify-between mb-2">
                                <p className={`text-[10px] font-black uppercase tracking-widest ${updateStatus.available ? "text-emerald-400" : "text-primary-500"}`}>
                                    {updateStatus.available ? "Update Available" : "Private Beta"}
                                </p>
                                {!updateStatus.available && (
                                    <div className="p-1 rounded-md bg-slate-800/80 text-slate-400 group-hover:text-white transition-colors">
                            ...
                            <p className="text-[10px] text-slate-500 group-hover:text-slate-300 mt-2 font-bold transition-colors">
                                {updateStatus.available ? "Click to upgrade" : "Info del proyecto & creador →"}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <main className="flex-grow flex flex-col overflow-y-auto relative">
                                                        ...
                                                        <div key={idx} className="bg-slate-900/40 border border-slate-800/50 rounded-[2rem] overflow-hidden group hover:border-emerald-500/30 transition-all flex flex-col justify-between">
                                                            <div className="relative h-44 w-full bg-slate-950 overflow-hidden">
                                                                ...
                                                                <div className="absolute top-3 right-3 flex items-center gap-2">
                                                                    ...
                                                                    <a
                                                                        href={post.shortcode ? `https://www.instagram.com/p/${post.shortcode}/` : (post.id ? `https://www.instagram.com/p/${post.id}/` : "https://www.instagram.com/")}
                                                                        target="_blank"
                                                                        rel="noreferrer"
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 backdrop-blur-md bg-black/60 animate-in fade-in duration-300">
                        <div className="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
                            ...
                            <div className="p-8 space-y-6">
                                <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800 space-y-2 text-slate-300 text-sm leading-relaxed">
                                    <p className="font-bold text-white flex items-center gap-2">
                                        <Mail className="w-4 h-4 text-emerald-400" /> Envíanos tus comentarios
                                    </p>
                                    <p className="text-slate-400 text-xs">
                                        ¿Tienes una idea para una nueva función, reportar un error o compartir tu experiencia? Tu opinión directa nos ayuda a evolucionar la herramienta.
                                    </p>
                                </div>

                                <div className="space-y-3">
                                    <a
                                        href={`mailto:cristianfieiras@gmail.com?subject=Sugerencia%20SocialRadar%20v${currentVersion}`}
                                        className="w-full py-4 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-3 shadow-lg shadow-emerald-600/20 hover:scale-[1.02] active:scale-[0.98]"
                                        onClick={() => setShowFeedbackModal(false)}
                                    >
                                        <Mail className="w-4 h-4" /> Enviar email a cristianfieiras@gmail.com
                                    </a>

                                    <button
                                        onClick={() => setShowFeedbackModal(false)}
                                        className="w-full py-3 rounded-xl font-bold text-slate-500 hover:text-slate-300 transition-colors text-xs uppercase tracking-wider"
                                    >
                                        Cerrar
                                    </button>
                                </div>
                ...
                {/* About Project & Creator Modal */}
                {showAboutModal && (
                    <div className="fixed inset-0 z-[110] flex items-center justify-center p-8 backdrop-blur-md bg-black/70 animate-in fade-in duration-300">
                        <div className="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
                            <div className="p-8 border-b border-slate-800 flex items-center justify-between bg-gradient-to-r from-primary-900/30 via-slate-900 to-transparent">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-2xl border border-white/10 bg-white/5 flex items-center justify-center shadow-lg shadow-primary-500/10 overflow-hidden">
                                        <img src={socialRadarLogo} alt="SocialRadar logo" className="w-9 h-9 object-contain" />
                                    </div>
                                    <div>
                                        <div className="flex items-center gap-2">
                                            <h2 className="text-xl font-black text-white tracking-tight">SocialRadar</h2>
                                            <span className="px-2.5 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 text-[10px] font-black uppercase tracking-wider">
                                                Private Beta
                                            </span>
                                        </div>
                                        <p className="text-xs text-slate-400 font-bold mt-0.5">Versión {currentVersion}</p>
                                    </div>
                                </div>
                                <button
                            ...
                            <div className="p-8 space-y-6">
                                ...
                                <div className="space-y-2">
                                    <h4 className="text-[10px] font-black uppercase tracking-widest text-primary-400">Sobre el Proyecto</h4>
                                    <p className="text-sm text-slate-300 leading-relaxed font-medium">
                                        SocialRadar es una suite avanzada de crecimiento orgánico y análisis para Instagram con automatización inteligente de comportamiento humano, protección anti-shadowban y sincronización en la nube.
                                    </p>
                                </div>
                                ...
                                <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                                    <p className="text-xs font-black text-white flex items-center gap-2">
                                        <Shield className="w-4 h-4 text-amber-400" /> Estado: Private Beta Activa
                                    </p>
                                    <p className="text-xs text-slate-400 leading-relaxed">
                                        Esta versión está en desarrollo continuo con mejoras frecuentes en seguridad, algoritmos de interacción y analítica.
                                    </p>
                                </div>

                                {/* Creator Info */}
                                <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Creador & Desarrollador</p>
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-primary-500/10 border border-primary-500/20 flex items-center justify-center text-primary-400 font-black text-sm">
                                                CF
                                            </div>
                                            <div>
                                                <p className="text-sm font-bold text-white">Cristian Fieiras</p>
                                                <a href="mailto:cristianfieiras@gmail.com" className="text-xs text-slate-400 hover:text-primary-400 transition-colors">
                                                    cristianfieiras@gmail.com
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-3 pt-2">
                                    <a
                                        href="https://github.com/cfieiras/SocialRadar"
                                        target="_blank"
                                        rel="noreferrer"
                                        className="w-full py-4 rounded-2xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-3 shadow-lg hover:scale-[1.02] active:scale-[0.98]"
                                    >
                                        <Github className="w-4 h-4 text-white" /> Ver Repositorio en GitHub <ExternalLink className="w-3.5 h-3.5 text-slate-400" />
                                    </a>

                                    <button
                                        onClick={() => setShowAboutModal(false)}
                                        className="w-full py-2.5 rounded-xl font-bold text-slate-500 hover:text-slate-300 transition-colors text-xs uppercase tracking-wider"
                                    >
                                        Cerrar
                                    </button>
                                </div>
